...
if($REQUEST_URI != '')
{
    ...
    $ARR_REQUEST_URI = parse_url($REQUEST_URI);
    $path = $ARR_REQUEST_URI['path'];
    $path = substr ($path , 0 , strpos($path,'plugins/ResponsiveFilemanager/filemanager'));
    //echo "<pre>".$path;
    //print_r($ARR_REQUEST_URI);
    //exit();
}
else
{
...
$config = array(
    ...
    */
    ...
    |
    */
    'upload_dir' => $path.'files/cms_images/source/',
    /*
    |--------------------------------------------------------------------------
    ...
    |
    */
    'current_path' => '../../../files/cms_images/source/',
